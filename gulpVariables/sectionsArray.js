module.exports = [
  {
    sectionTitle: "Navigation section",
    sectionSrcPathArray: ["03_HTML/01_Navbar/"],
    sectionIncludedTemplateString: "01_Navbar.njk",
    sectionOutputFileName: "01_Navbar.html",
    cssFilePath: "../01_CSS/01_Navbar",
    cssFilesArray: [
      "01_Navbar",
      "01_Navbar_Hamburger",
      "01_Navbar_media_queries",
    ],
  },
  {
    sectionTitle: "Carousel section",
    sectionSrcPathArray: ["03_HTML/02_Carousel/"],
    sectionIncludedTemplateString: "02_Carousel.njk",
    sectionOutputFileName: "02_Carousel.html",
    cssFilePath: "../01_CSS/02_Carousel",
    cssFilesArray: ["02_Carousel", "02_Carousel_media_queries"],
  },
  // {
  //   sectionTitle: "Cells section",
  //   sectionSrcPathArray: ["03_HTML/03_Cells/", "03_HTML/03_Cells/icons"],
  //   sectionIncludedTemplateString: "03_Cells.njk",
  //   sectionOutputFileName: "03_Cells.html",
  //   cssFilePath: "../01_CSS/03_Cells",
  //   cssFilesArray: ["03_Cells", "03_Cells_media_queries"],
  // },
  {
    sectionTitle: "About section",
    sectionSrcPathArray: ["03_HTML/04_About/"],
    sectionIncludedTemplateString: "04_About.njk",
    sectionOutputFileName: "04_About.html",
    cssFilePath: "../01_CSS/04_About",
    cssFilesArray: ["04_About", "04_About_media_queries"],
  },
  // {
  //   sectionTitle: "Testimonials section",
  //   sectionSrcPathArray: ["03_HTML/05_Testimonials/"],
  //   sectionIncludedTemplateString: "05_Testimonials.njk",
  //   sectionOutputFileName: "05_Testimonials.html",
  //   cssFilePath: "../01_CSS/05_Testimonials",
  //   cssFilesArray: ["05_Testimonials", "05_Testimonials_media_queries"],
  // },
  {
    sectionTitle: "Staff section : Quotations",
    sectionSrcPathArray: ["03_HTML/06_Staff/"],
    sectionIncludedTemplateString: "06_Staff_quotations.njk",
    sectionOutputFileName: "06_Staff_quotations.html",
    cssFilePath: "../01_CSS/06_Staff",
    cssFilesArray: ["06_Staff", "06_Staff_media_queries"],
  },
  {
    sectionTitle: "Staff section : Detailed",
    sectionSrcPathArray: ["03_HTML/06_Staff/"],
    sectionIncludedTemplateString: "06_Staff_detailed.njk",
    sectionOutputFileName: "06_Staff_detailed.html",
    cssFilePath: "../01_CSS/06_Staff",
    cssFilesArray: ["06_Staff", "06_Staff_media_queries"],
  },
  {
    sectionTitle: "Updates section",
    sectionSrcPathArray: ["03_HTML/07_Updates/"],
    sectionIncludedTemplateString: "07_Updates.njk",
    sectionOutputFileName: "07_Updates.html",
    cssFilePath: "../01_CSS/07_Updates",
    cssFilesArray: ["07_Updates", "07_Updates_media_queries"],
  },
  {
    sectionTitle: "Footer section",
    sectionSrcPathArray: ["03_HTML/08_Footer/", "03_HTML/08_Footer/icons"],
    sectionIncludedTemplateString: "08_Footer.njk",
    sectionOutputFileName: "08_Footer.html",
    cssFilePath: "../01_CSS/08_Footer",
    cssFilesArray: ["08_Footer", "08_Footer_media_queries"],
  },
  {
    sectionTitle: "Welcome section",
    sectionSrcPathArray: ["03_HTML/09_Banner/"],
    sectionIncludedTemplateString: "09_Banner.njk",
    sectionOutputFileName: "09_Banner.html",
    cssFilePath: "../01_CSS/09_Banner",
    cssFilesArray: ["09_Banner", "09_Banner_media_queries"],
    banner_superscript: `Welcome to`,
    banner_header: `The Hill Preparatory School`,
    banner_subscript: `We believe in holistic child development that turns every student into an active global citizen.`,
  },
  {
    sectionTitle: "Content A : A",
    sectionSrcPathArray: ["03_HTML/11_Content/"],
    sectionIncludedTemplateString: "11_Content_A.njk",
    sectionOutputFileName: "11_Content_A.html",
    cssFilePath: "../01_CSS/11_Content",
    cssFilesArray: ["11_Content", "11_Content_media_queries"],
  },
  {
    sectionTitle: "Content B : B",
    sectionSrcPathArray: ["03_HTML/11_Content/"],
    sectionIncludedTemplateString: "11_Content_B.njk",
    sectionOutputFileName: "11_Content_B.html",
    cssFilePath: "../01_CSS/11_Content",
    cssFilesArray: ["11_Content", "11_Content_media_queries"],
  },
  {
    sectionTitle: "Content C : Curriculum",
    sectionSrcPathArray: ["03_HTML/11_Content/"],
    sectionIncludedTemplateString: "11_Content_C.njk",
    sectionOutputFileName: "11_Content_C_Curriculum.html",
    cssFilePath: "../01_CSS/11_Content",
    cssFilesArray: ["11_Content", "11_Content_media_queries"],
  },
  {
    sectionTitle: "Content D : Video",
    sectionSrcPathArray: ["03_HTML/11_Content/"],
    sectionIncludedTemplateString: "11_Content_D.njk",
    sectionOutputFileName: "11_Content_D_Video.html",
    cssFilePath: "../01_CSS/11_Content",
    cssFilesArray: ["11_Content", "11_Content_media_queries"],
  },
  {
    sectionTitle: "Subjects",
    sectionSrcPathArray: ["03_HTML/12_Subjects/"],
    sectionIncludedTemplateString: "12_Subjects.njk",
    sectionOutputFileName: "12_Subjects.html",
    cssFilePath: "../01_CSS/12_Subjects",
    cssFilesArray: ["12_Subjects", "12_Subjects_media_queries"],
  },
];
